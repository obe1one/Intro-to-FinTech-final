...
def rrEstimate(file_dailyOhlcv, file_minutelyOhlcv):
    dailyOhlcv = pd.read_csv(file_dailyOhlcv)
    minutelyOhlcv = pd.read_csv(file_minutelyOhlcv)
    capital = 500000.0
    capitalOrig=capital
    transFee = 100
    evalDays = 14
...
def main():
    file_dailyOhlcv = sys.argv[1]
    file_minutelyOhlcv = sys.argv[2]
   # print(rrEstimate(file_dailyOhlcv, file_minutelyOhlcv))
